...
def get_default_students():
    ...
    for student_id in range(101921, 101941):
        ...
        # Randomly assign one of the available activities (from keys 2001 to 2007)
...
# -------------------------------
# Global Data (loaded or default)
# -------------------------------
activities, students = load_data()
# If file did not exist, save the default data for future runs.
if not os.path.exists(DATA_FILE):
    save_data(activities, students)

# -------------------------------
# Hard-coded user credentials (username -> {password, role, student_id?})
# (These remain hard-coded; only a few student logins are provided.)
# -------------------------------
USERS = {
...
# -------------------------------
# Sample data for teachers (unchanged)
# -------------------------------
teachers = {
...
# -------------------------------
# Utility function to display student info in a side panel (used by Admin and Staff)
# -------------------------------
...
# -------------------------------
# Main Application Window with Logout functionality
# -------------------------------
class MainApplication(tk.Tk):
    def __init__(self, role, student_id=None):
        ...
        # Container frame for role-specific content
...
# -------------------------------
# Admin Frame (Administrator role) with side-panel details
# -------------------------------
class AdminFrame(ttk.Frame):
    def __init__(self, parent):
        super().__init__(parent, padding=10)
        # Split into left (activities) and right (details/editor)
        ...
        # Right frame will display enrolled students or editor form.
    ...
    def on_activity_double_click(self, event):
        ...
        if not selection:
            return
        item = selection[0]
        row = self.activity_tree.item(item, "values")
        if not row:
            return
        act_id = int(row[0])
    ...
    def show_enrolled_students(self, activity_id):
        ...
        # When a student is double-clicked, show their info below
...
# -------------------------------
# Staff Frame (Teacher role) with side-panel details for students
# -------------------------------
class StaffFrame(ttk.Frame):
    ...
    def on_student_double_click(self, event):
        ...
        if selection:
            item = self.st_tree.item(selection[0], "values")
            student_id = int(item[0])
            info = format_student_info(student_id)
            self.info_label.config(text=info)

# -------------------------------
# Student Frame (Student role)
# -------------------------------
class StudentFrame(ttk.Frame):
    def __init__(self, parent, student_id):
        ...
        ttk.Label(self, text="Currently Enrolled Activities:").pack(anchor=tk.W, pady=5)
        self.enrolled_listbox = tk.Listbox(self, height=5)
        self.enrolled_listbox.pack(fill=tk.X, pady=5)
        ttk.Label(self, text="All Available Activities:").pack(anchor=tk.W, pady=5)
    ...
    def refresh_lists(self):
        ...
        for a_id in enrolled:
            act_name = activities[a_id]["activity"] if a_id in activities else "Unknown"
            self.enrolled_listbox.insert(tk.END, f"{a_id} - {act_name}")
        for a_id, a_data in activities.items():
            if a_id not in enrolled:
    ...
    def enroll_in_activity(self):
        ...
        if not selection:
            return
        line = self.activities_listbox.get(selection[0])
        try:
            a_id = int(line.split()[0])
        except ValueError:
        ...
        if not student_data:
            ...
            return
        if a_id not in student_data.get("activities_enrolled", []):
